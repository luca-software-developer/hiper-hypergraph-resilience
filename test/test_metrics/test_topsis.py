...
Unit tests for the TopsisNodeRanker class.
Tests TOPSIS multi-criteria node ranking including criteria computation,
ranking algorithms, node selection, and weight handling for various hypergraph
structures.
"""
...
class TestTopsisNodeRanker(unittest.TestCase):
    """
    Tests TopsisNodeRanker methods.
    Methods tested include node ranking computation, criteria calculation,
    top/bottom node selection, and weight customization.
    """

    def setUp(self):
        """Create TOPSIS ranker and test hypergraphs."""
        self.ranker = TopsisNodeRanker()
    ...
    def test_ranking_score_bounds(self):
        """All ranking scores should be in valid range [0, 1]."""
        ranked_nodes = self.ranker.rank_nodes(self.hn_complex)
    ...
    def test_matrix_normalization(self):
        """Matrix normalization should produce unit column vectors."""
        # Create test matrix
        ...
        for j in range(normalized.shape[1]):
            ...
            self.assertAlmostEqual(column_norm, 1.0, places=5)

    def test_topsis_algorithm_components(self):
        """TOPSIS algorithm components should work correctly."""
        ...
        weights = [1.0, 1.0, 1.0]
        scores = self.ranker._apply_topsis(criteria_matrix, weights)
    ...
    def test_criteria_bounds_validation(self):
        ...
        for node_id in nodes[:3]:  # Test subset to avoid excessive computation
